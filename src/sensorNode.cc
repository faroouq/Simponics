...
{
    sensors[0]="temp?";                                         //Telling the MCU the sensors connected
    sensors[1]="humd?";
...
void sensorNode::handleMessage(cMessage *msg)
{
    if(strcmp(msg->getName(), "Sense")==0){ //****************It is sensing time************
        ...
        this->energy-=this->i_tx*this->v_trx*tx_time;                   //Energy for data transmission
 ...
 *******************************************************************************************/
double sensorNode::min (double x, double y){
    if(x<y)
        return x;
    else
        return y;
